...
import './index.scss';
import { publications, urqlClient } from 'utils/urql-client';

const ThreadsModal = ({ showModal }) => {
  const background = useRef<HTMLDivElement>(null);
  const [threads, setThreads] = useState<string[]>([]);

  const getServerResonse = async () => {
    const response = await urqlClient
      .query(publications, '' as any)
      .toPromise();
    const res: string[] = [];
    const items = response.data.publications.items;
    for (let i = 0; i < items.length; i++) {
      if (items[i].__typename === 'Post' && res.length < 4) {
        res.push(items[i]);
      }
    }
    setThreads(res);
        ...
        <div className='ThreadsModal-container'>
          ...
          <div className='ThreadsModal-content'>
            {threads.map((thread, index) => {
              const content = thread.metadata.content
                .replace('**', '')
                .replace('**', '')
                .replace('**', '')
                .replace('**', '');
              const skipToLine = content.indexOf(']');
              const contentTitle = content.slice(0, skipToLine + 3);
              const contentWithoutTitle = content.slice(skipToLine + 8);

              return (
                <a
                  href={`https://lenster.xyz/posts/${thread.id}`}
                  target='_blank'
                  rel='noreferrer'
                  className='ThreadsModal-content-thread'
                  key={index}
                >
                  <div className='ThreadsModal-content-thread-title'>
                    {contentTitle}
                  </div>
                  <div className='ThreadsModal-content-thread-content'>{contentWithoutTitle}</div>
                </a>
              );
            })}
          </div>
